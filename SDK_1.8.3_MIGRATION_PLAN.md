# SDK 1.8.3 Migration Plan

## Summary

SDK version 1.8.3 has been published with significant improvements:
...
#### Changes Needed:
1. **Remove format detection** - SDK 1.8.3 handles this internally
2. **Use separate ODCL parser** - Call `parse_odcl_yaml()` instead of `parse_odcs_yaml()` for ODCL files
3. **Remove merge logic** - SDK 1.8.3 returns complete data
4. **Simplify parseYAML()** - Direct SDK calls without pre/post-processing
5. **Remove validateTableCompleteness()** - SDK 1.8.3 handles validation
...
SDK 1.8.3 supports more SQL dialects. Current UI shows:
- PostgreSQL
- MySQL  
- SQLite
- SQL Server
...
**Action**: Check SDK 1.8.3 documentation for new dialect support and update the dropdown if needed. Common additional dialects might include:
- Oracle
- Snowflake
- BigQuery
- Redshift
...
2. Update binding verification to check for 1.8.3+ methods
3. Update WASM build scripts to use SDK 1.8.3
...
### Low Risk
- SDK 1.8.3 is more complete than previous versions
...
4. `frontend/public/wasm/*` - Update to SDK 1.8.3 WASM build
...
1. Verify SDK 1.8.3 is available and accessible
